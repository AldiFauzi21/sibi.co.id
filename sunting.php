<?php
  ...
  if(isset ($_POST ['unggah'])) {
    ...
    fwrite($infokom, "${isi}");
    fclose($infokom);
    if(sunting($judul, $instansi, $id)>0)
          echo "<script>alert('Data berhasil disimpan');</script>";
          header("location:baca.php?id_info=$id_informasi");
    }
  }
?>
<!doctype html>
          ...
          <div class="form-group">
            <form action="" method="POST" enctype="multipart/form-data">
              <label for="judul">Judul</label>
                <input type="text" name="judul" class="form-control"  placeholder="Judul Informasi" value="<?=$info['judul'];?>"> 
              </div>
                ...
                <label for="isi">Isi</label>
                  ...
                  <div class="form-group">
                    <label for="instansi">Instansi</label>
                        <input type="text" name="instansi" class="form-control" placeholder="Instansi Anda" value="<?=$info['instansi'];?>">
                      </div>

                      <!--Upload gambar-->
                        <button type="submit" name="unggah" class="btn btn-primary">Sunting</button><a href="Infokom.php" class="btn btn-primary btn-lg active" role="button" aria-pressed="true" style="background-color: #DC143C ; border-color: #DC143C; font-size: 11pt; " autocomplete="on">Kembali</a>
                  </form></div></div></div>
